...
// Problem 1
//find all runners from any given company
let getCompany = [];
function runnersCompany(company){
    runners.filter(function(runner){
        if(runner.company_name=== company){
            getCompany.push(JSON.stringify(runner));
        }
    });
    console.log(getCompany);
}

runnersCompany('Yombu');
// Problem 2
//sort emails alphabetically
const alphaEmails = runners.map(function(runner){
    return runner.email;
}).sort();

console.log(alphaEmails);

// Problem 3
//find a runner's ID #
function runnersNum (first, last){
    runners.forEach(function(runner){
        if(runner.first_name === first && runner.last_name === last){
            console.log("This is " + first +"'s" + " id#:", runner.id);
            return runner.id;
        }
    });
}

runnersNum('Sarina', 'Lasham');